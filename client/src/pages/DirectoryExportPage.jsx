...
export default function DirectoryExportPage() {
  ...
  const handleExport = async (format) => {
    ...
    try {
      ...
      if (format === 'csv') {
        ...
        blob = await exportService.exportDirectoryCSV(filters);
        filename = `employee-directory-${new Date().toISOString().split('T')[0]}.csv`;
      } else if (format === 'excel') {
        setExportingExcel(true);
        blob = await exportService.exportDirectoryExcel(filters);
        filename = `employee-directory-${new Date().toISOString().split('T')[0]}.xlsx`;
      } else if (format === 'pdf') {
        setExportingPDF(true);
        blob = await exportService.exportDirectoryPDF(filters);
        filename = `employee-directory-${new Date().toISOString().split('T')[0]}.pdf`;
      }

      exportService.downloadFile(blob, filename);
    } catch (err) {
      console.error(`Failed to export as ${format}:`, err);
      setError(`Failed to export employee directory. Please try again.`);
    } finally {
  ...
  return (
    <Box sx={{ px: 3, py: 2 }}>
      ...
      <Typography variant="body1" sx={{ mb: 4, color: 'text.secondary' }}>
        Export a flat list of all employees with their details
      </Typography>
